 ...
 */
int dosfs_init(struct fsys_desc* fs)
{
   uint c;
...
#ifdef VERIFY_FAT
static uint _fatread(uint c)
{
   return(pfat[c]);
}
#define FAT(x) pfat[x]

static void verify_fat(uint i, uint val, ushort* p)
{
   if(_fatread(i) != val)
   {
      uint c;
...
int fatwrite(uint i, ushort val)
{
   struct buffer_head* bh;
   ushort* p;
   uint fatblk, off;
...
int dosfs_read_fat(struct buffer_head* bh)
{
   ASSERT(bh->id == BT_FAT);
   reads(bh->pbuf, fatstart + (bh->blk * secperfatblk), secperfatblk);
}

int dosfs_write_fat(struct buffer_head* bh)
{
   ASSERT(bh->id == BT_FAT);
...
int dosfs_read_root(struct buffer_head* bh)
{
   ASSERT(bh->id == BT_ROOT);
   reads(bh->pbuf, rootsec, SECPER_ROOT);
}

int dosfs_write_root(struct buffer_head* bh)
{
   ASSERT(bh->id == BT_ROOT);
...
int dosfs_read_block(struct buffer_head* bh)
{
...
int dosfs_write_block(struct buffer_head* bh)
{
   ASSERT(bh->id == BT_REGULAR);
 ...
 */
static void dosfs_name(struct dir_entry* dirent, char* buf)
{
   char* p;
   char ext[4];
 ...
 */
int dosfs_lookup(struct fcb* dir, char* name, struct inode* res)
{
...
int dosfs_read(struct fcb* f, void* p, int cb)
{
   uint blksz, blkno;
   uint offset, left;
   uint read = 0;
...
int dosfs_write(struct fcb* f, void* p, int cb)
{
   uint blksz;
   int blkno;
   uint offset, left;
...
void dosfs_setroot(struct fcb* dir)
{
 ...
 * set given directory
 */
int dosfs_setpwd(struct fcb* dir, char* name)
{
   char buf[20];
   uint blk;
   struct inode i;
...
static uint dosfs_write_node(struct fcb* dir, char* name, struct inode* i)
{
   struct dir_entry dirent;
   uint nblk;
   uint last_blk;
...
int dosfs_mknode(struct fcb* dir, char* name, uint type, struct inode* res)
{
   struct inode i;
...
int dosfs_truncate(struct fcb* f)
{
	uint blk, last_blk;
